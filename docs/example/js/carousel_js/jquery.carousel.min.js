(function(c) {
	function ga(z, s) {
		...
		function ha(b) { 
			...
			var d = c('<div class="carousel-item"><img /><div class="txt">'+j[b].txt+'</div></div>').appendTo(i);
		...
		function o(b) {
			...
			i.find(".carousel-item").each(function(d) {
				...
				//增加形变效果
				var l = Math.cos(-(p * d) + v * 0.5 + b * w) * a.settings.horizontalRadius + Q - h * 0.5 + h * (1 - r) / 2;
				// var newl = l (a.settings.horizontalRadius *0.5 + a.settings.itemWidth * 0.5)
				var leftPosW = a.settings.itemWidth * ((1-a.settings.scaleRatio)/2) *2
				// console.log(leftPosW)
				var minL = (a.settings.width - a.settings.horizontalRadius*2)/2;
				// console.log('minL:'+minL);
				var maxL = (minL+a.settings.horizontalRadius*2 - leftPosW);
				// console.log('maxL：'+maxL);

				//左偏移的范围是[minL,maxL] 
				//给元素增加 transform: perspective(800px) rotateY(-30deg);
				//rotateY从30~-30进行变化
				var rate = 80 / (maxL - minL);
				// console.log('rate:'+rate);
				// console.log(l);

				var deg = 40 - (l - minL) * rate; 



				//增加透明度效果
				var t = D + f * (1 - r) / 2;
				// var topPosH = a.settings.itemHeight * ((1-a.settings.scaleRatio)/2) *2
				// var minT = (a.settings.height - a.settings.verticalRadius*2)/2;
				// var maxT = (minT+a.settings.verticalRadius*2 - topPosH);
				// console.log(topPosH)
				// console.log(minT)
				// console.log(maxT)
				...
				var rate1 = 0.8/(maxT - minT);
				console.log('rate1:'+rate1);
				var opacity  = 0.8 - (t - minT)*(rate1>0?rate1:0)
			...
			if (a.settings.xmlSource) {
				...
				i.children().each(function() {
					for (var b = {}, d = 0; d < c(this).children().length; d++) {
						...
						} else if (c(e).is("img")){ b.path = c(e).attr("src"); b.div = c(e).find('.txt');}
